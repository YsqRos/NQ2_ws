...
// const unsigned char header[5] = {0xaa, 0x20, 0x20, 0x42, 0x06};
...
//         v_x.d=RobotV_x*100.0;
//         v_y.d=RobotV_y*100.0;
//         v_z.d=YawRate*100.0;
...
//     for(i = 0; i < 2; i++)
//     {
//         buf[i + 5] = v_x.data[i];  //buf[5] buf[6]
//         buf[i + 9] = v_y.data[i];  //buf[9] buf[10]
//         buf[i + 11] = v_z.data[i]; //buf[11] buf[12]
//     }
...
// {
// 	RobotVx_ = msg.linear.x ;
//     RobotVy_ = msg.linear.y ;
...
//     else
//     {
//         return -1;ROS_INFO_STREAM("/dev/ttyUSB1 is o666.");
//     }
...
//         // unsigned char buf[16] = {0xAA, 0x20, 0x20, 0x42, 0x06, 0x68, 0x00, 0x68, 0x00, 0x68, 0x00, 0x68, 0x00, 0x0D};
//         // sp.write(buf,16);
//         ros::spinOnce();
//         spinOnce(RobotVx_,RobotVy_,YawRate_);
//         loop_rate.sleep();
//     }
...
#include <geometry_msgs/TransformStamped.h>
#include <geometry_msgs/Twist.h>
#include <iomanip>  // 添加iomanip用于格式化输出
using namespace serial;
serial::Serial sp;
 double RobotVx_ = 0;
 double RobotVy_ = 0;
...
void writeSpeed(double RobotV_x, double RobotV_y,double YawRate)
{
    ...
    // double r = RobotV / YawRate;//mm
     unsigned short Crc16Value;
        v_x.d=RobotV_x/0.22*500.0*1.05;
        v_y.d=RobotV_y/0.22*500.0*1.05;
        v_z.d=YawRate/0.22*500.0*1.05;
    ...
    {
        buf[i + 5] = v_x.data[i];  //buf[5] buf[6]
        buf[i + 7] = v_y.data[i];  //buf[9] buf[10]
        buf[i + 9] = v_z.data[i]; //buf[11] buf[12]
    }

    // 设置校验值、消息尾
    Crc16Value = getCrc16(buf, 11);
    buf[11] = (unsigned char)Crc16Value;   // buf[13]
    buf[12] = (unsigned char)(Crc16Value >> 8);  // buf [14]
    buf[13] = ender[0];   //buf[15]
    
    // 添加ROS_INFO打印buf的值
    std::stringstream ss;
    ss << "Sending buffer: ";
    for(int j = 0; j < 14; j++) {
        ss << std::hex << std::setfill('0') << std::setw(2) << (int)buf[j] << " ";
    }
...
int main(int argc, char** argv)
{
    ...
    {
        // unsigned char buf[16] = {0xAA, 0x20, 0x20, 0x3A, 0x08, 0xE8, 0x03, 0x18, 0xFC, 0x18, 0xFC, 0xE8, 0x03, 0xDE, 0x79, 0x0D};
         //unsigned char buf[16] = {0xAA, 0x20, 0x20, 0x42, 0x06, 0x64, 0x00, 0x64, 0x00, 0x64, 0x00, 0x3B, 0x6A, 0x0D};
         
        // 添加ROS_INFO打印buf的值
        std::stringstream ss;
        ss << "Sending fixed buffer: ";
        for(int j = 0; j < 14; j++) {
            ss << std::hex << std::setfill('0') << std::setw(2) << (int)buf[j] << " ";
        }
